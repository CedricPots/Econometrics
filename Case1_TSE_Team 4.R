...
#   Only Weekends
      ...
      plot(0:23, average_wknd_hr, type="l", lwd=2, ylab= "Average Hourly Demand", xlab="Hour", 
           main="Average Hourly Demand on Weekdays (Sat-Sun)")
      abline(v=8, col="red", lty=2, lwd=2)
      abline(v=17, col="blue", lty=2, lwd=2)
...
# We plot the "best" model, the SARIMA(1,0,0)(2,1,2):
# Finally, we plot the fit on the training data for both models:
plot(train_ts[1:(24*7*4)], type="l", col="#006666", ylab="Hourly Demand",
     main= "True and Fitted Values for ARIMA(1,0,0)(2,1,2)", frame.plot = FALSE)
lines(fitted(sarima100212)[1:(24*7*4)], col="#CC6600")
legend(100, 1100, legend=c("True Value", "Fitted Value"),
       lty=c(1,1), col=c("#006666", "#CC6600"), bty="n", ncol=2, 
       xpd=TRUE)
...
# Plotting Rolling Window Forecasts:
  ...
  # We plot the "best" model, the SARIMA(2,0,0)(0,1,3):
  # Finally, we plot the fit on the training data for both models:
  plot(train_ts[1:(24*7*4)], type="l", col="#006666", ylab="Hourly Demand",
       main= "True and Fitted Values for ARIMA(2,0,0)(0,1,3) using rolling window", 
       frame.plot = FALSE, ylim = c(min(rw_hourly_forecast_SARIMA200013[1:(24*7*4),2]), max(rw_hourly_forecast_SARIMA200013[1:(24*7*4),2])))
  lines(rw_hourly_forecast_SARIMA200013[1:(24*7*4),2], col="#CC6600")
  legend(100, 1500, legend=c("True Value", "Fitted Value"),
         lty=c(1,1), col=c("#006666", "#CC6600"), bty="n", ncol=2, 
         xpd=TRUE)
...
# Finally, we plot the fit on the training data for both models:
plot(train_ts[1:(24*7*4)], type="l", col="#006666", ylab="Hourly Demand",
     main= "True and Fitted Values for ARIMA(2,0,0)(0,1,3) using expanding window", 
     frame.plot = FALSE, ylim = c(min(rw_hourly_forecast_SARIMA200013[1:(24*7*4),2]), max(rw_hourly_forecast_SARIMA200013[1:(24*7*4),2])))
lines(ew_hourly_forecast_SARIMA200013[1:(24*7*4),2], col="#CC6600")
legend(100, 1500, legend=c("True Value", "Fitted Value"),
       lty=c(1,1), col=c("#006666", "#CC6600"), bty="n", ncol=2, 
       xpd=TRUE)
...
mcs_absolute <- MCSprocedure(Loss = absolute_loss_matrix, alpha = 0.3, B = 5000, 
                             statistic = "Tmax")

mcs_squared <- MCSprocedure(Loss = squared_loss_matrix, alpha = 0.3, B = 5000, 
                            statistic = "Tmax")
